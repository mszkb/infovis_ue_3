...
function plotLineChart(){
    ...
    d3.csv("/static/data/dataAggregatedPivot.csv",
      ...
      function(data) {
        ...
        var dataRegion = data.map(function(d){return {date: d.date, value:d[selectedRegion]} })

        // add x axis to the coordinate system
            xdomain=d3.extent(dataRegion, function(d) { return d.date; }) //save the domain to xdomain to use it for drawing the vertical lines later
        ...
        // add y axis to the coordinate system
            var y = d3.scaleLinear()//use d3.scaleLinear to map [0,max] of the values to the available height of the coordinate system (the maximum value is given in the data of China)
        ...
        // Draw the initial lineChart (based on the initially selected region defined in dataRegion (EU))
         line.append("path")
          ...
          .attr("d", d3.line()
            .x(function(d) { return x(d.date) }) //use the above-defined scaling of time (x)
            .y(function(d) { return y(d.value) }) //use the above-defined scaling of gold (y)
            )
        ...
        function brushChart() {
          ...
          if(extent){
            ...
            line.select(".brush").call(brush.move, null) // This removes the grey brush area as soon as the selection has been done
              ...
              .duration(1500)
              .attr("d", d3.line()
                .x(function(d) { return x(d.date) }) //scale the x values with the new domain scaling
              ...
              vertLine
                  ...
                  .transition()
                  .duration(1500)
                  .attr("x1",function(d) { return x(d.date) }) //scale the x values with the new domain scaling
                    .attr("y1",0)
        ...
        svg.on("dblclick",function(){
          x.domain(d3.extent(data, function(d) { return d.date; }))//redefine the x-axis scaling based on the whole data
        ...
        function updateRegion(selectedRegion) {
          ...
          dataRegion = data.map(function(d){return {date: d.date, value:d[selectedRegion]} })
    ...
    //read the blizzard release data to draw the vertical lines that correspond to the release dates (games & patches)
      ...
      function(d){
        return { date : d3.timeParse("%Y-%m-%d")(d.release),
            title: d.title,
            company: d.company,
            type: d.type}
      },
